/*
AjaxZip3の拡張版
JCM-SYSTEMでは、select2というプラグインを入れている為、
都道府県のプルダウンに特殊な値の入れ方を行う為、拡張している。
それ以外は、AjaxZip3と変わらない。

都道府県がプルダウンの場合、プルダウン側に必ずidを振ること。
*/
...
AjaxZip3.callback = function(data){
        ...
        if ( AjaxZip3.fpref.type == 'select-one' || AjaxZip3.fpref.type == 'select-multiple' ) {
            // 都道府県プルダウンの場合(select2バージョン)
            $('#'+AjaxZip3.fpref.id).val(pref_id).change();
